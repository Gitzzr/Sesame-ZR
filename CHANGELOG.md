...
| 日期 | 模块 | 功能改动 |
| --- | --- | --- |
| 2026-09-24 | 任务调度 / 可观测性 | 任务执行统计结构化落盘：每轮执行结束时把「完成 / 后台启动 / 超时 / 失败 / 跳过」数量与总耗时写入按账号隔离的 `config/<userId>/statistics.json`，按自然日累积并保留最近 30 天；此前这些数字只以文本形式打印在日志里，无法直接用于分析。 |
| 2026-09-24 | 设置中心 / 多账号 | 新增「批量启用功能」：把选中的好友一键写进各功能的好友列表并打开对应开关，免去逐个功能配置；功能清单默认按推荐勾选，**勾了才写、没勾一律不动**。小号档不会动大号任何资源（不收大号能量由「收集能量」开关兜底），大号档会把选中的账号从「不收能量」等排除名单里移除 —— 大号需要收小号的能量。 |
