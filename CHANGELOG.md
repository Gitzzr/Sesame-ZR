...
| 2026-09-21 | RPC 安全验证 / 小鸡庄园 | 修复安全验证暂停卡死：通用业务拒绝码 1009 不再被误判为需要人工验证（此前普通业务失败会让任务被暂停，而支付宝并不弹验证页，用户只能卸载支付宝才能恢复）；暂停提示会指出触发风控的业务入口（如蚂蚁庄园），因为支付宝验证页与触发场景绑定，在首页找不到入口；暂停标志增加 30 分钟超时并在启动时自动解除，旧版本遗留的无时间戳标志视为过期；恢复流程改为账户一致即清除标志，不再因令牌校验分支把用户永久卡住；通知与支付宝首页确认框新增「跳过并恢复」入口，并改写提示文案说明无验证页时的处理方式；模块内部广播接收器在 Android 8.0–12L 上因导出级别设置不当而**注册失败**（原写 `RECEIVER_NOT_EXPORTED`，而该能力在 API 33 以下要求宿主 manifest 声明一个无法添加的权限，会直接抛异常），导致「手动任务 / 重启」等指令全部失效，现命令通道改为导出；同时把「恢复安全验证暂停 / 重新登录」拆到独立的自救通道，在 Android 13 及以上不再导出，并让恢复指令必须匹配一次性令牌，避免外部应用伪造广播强制解除暂停。 |
