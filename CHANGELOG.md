...
| 日期 | 模块 | 功能改动 |
| --- | --- | --- |
| 2026-09-23 | 设置中心 / 多账号 | 新增「账号档位（大号 / 小号）」一键切换（设置页 → 扩展&外观）。大号档为全功能档，开启各模块收益与进度类能力、同时保持贴罚单 / 丢肥料 / 请走小摊 / 通知赶鸡 / 打赏好友等纯干扰项关闭；小号档为隔离档，关闭全部 80 项以好友（含大号）为对象的设置并清空好友名单，只保留纯本账号的签到、任务与领取，确保小号不会窃取或消耗大号资源。两档都会把本机其他已载入账号写入「不收能量 / 小号列表 / 复活能量」三份保护名单，形成双向隔离。档位不触碰调度、日志与调优参数。隔离约束由单测强制校验：新增设置项若未按资源作用域（仅本账号 / 跨账号）登记，测试会失败。 |
