...
- [ ] 用户可见的改动 → 追加 `CHANGELOG.md` 表格**一行**（`| 日期 | 模块 | 功能改动 |`），见下节
...
本仓库的 `CHANGELOG.md` 是**日期表**、**没有 `Unreleased` 区段**，规范里「途中累积」那一段无处安放。等价规则是：

> `main` 永远可发布、tag / Release 都从 `main` 上切 ⇒ **合入 `main` 的那一刻就是这条改动的 release time**。
> 因此 CHANGELOG 写在整个 PR **收尾时**，**一个 PR 一行**。

- **不要跟着每个提交写**：同一个 PR 里连加 5 行是本仓库实际出现过的反例
- **一个 PR 只占一行**：改了几个模块也合成一行，模块列用 ` / ` 连接，不拆成多行
...
| 列 | 规则 |
| --- | --- |
| 日期 | 该 PR **合入 `main` 的日期**。写 PR 时先填当天，跨天不回头改 |
| 模块 | 用户能认出的功能名（`设置中心`、`小鸡庄园`），多个用 ` / ` 连接；**不写类名**（`AntForest`） |
| 功能改动 | **用户视角**的变化，用「新增 / 修复 / 调整 / 移除」起头；一句话一个变化，用 `；` 分隔 |

- **插入位置**：表格**最上面**（整体按日期倒序，同日多行相邻即可）
- **长度**：1~3 句、**≤ 200 字**（现有中位数约 110 字）
- **不写实现**：字段清单、踩坑过程、验证手法放 `docs/superpowers/{specs,plans}/`
- **不追溯**：已超长的历史行保持原样，不为新规则回头改写

自检（打印最长一行的字数与日期，应 ≤ 200）：

```bash
python -c "import io;print(max((len(l.split('|')[3].strip()), l.split('|')[1].strip()) for l in io.open('CHANGELOG.md',encoding='utf-8') if l.startswith('| 20')))"
```
