...
本项目已有的六份实施计划固定了一套工作流，建议沿用。
这套目录约定来自 superpowers 插件（`obra/superpowers`）里 `brainstorming` 与 `writing-plans`
两个 skill 的默认落盘位置：

| 产出 | 路径 |
| --- | --- |
| 设计稿 | `docs/superpowers/specs/YYYY-MM-DD-<topic>-design.md` |
| 实施计划 | `docs/superpowers/plans/YYYY-MM-DD-<topic>.md` |

因此下面两条约束要一起遵守：

1. **一个功能只有一对文档。** 需求变化、推翻重来、后续轮次，都在**既有文档里追加章节**
   （例：「十、2026-09-23 PR#3 评审核对与修复方案」），**不新开文件** —— 只有"换了一个功能"
   才开新的一对。同名文件的日期用**首次立项那天**，后续轮次不改名、不搬家。
   追加时顺带做一件事：**把被本轮推翻的旧表述就地改掉**（设计稿描述的是当前设计，不是历史）。
   分工是：`specs/` 写**当前**设计，`plans/` 写**过程**（只追加，不回头改结论）。
2. **文件名用「功能 / 问题名」，不要用 PR / Issue 编号。** `<topic>` 要是一个长期存在的工作对象
   （`account-preset`、`verification-pause-stuck`），不是 `pr3`、`issue-128` 这种一次性编号 ——
   文档的生命周期比 PR 长，PR 合掉之后那个名字就没有意义了。评审报告、核对结论、修复记录
   若针对的是**已存在的功能**，就并入该功能的文档（它们只是"某功能的某一轮"）。

> 反例（两处都真实发生过）：同一个功能分两轮各开一对文档，读者得先搞清"该看哪一份"；
> 把一份 PR 评审核对报告命名为 `2026-09-23-pr3-review-verification.md` 放进 `plans/`，
> 而它核对的正是 `2026-09-21-verification-pause-stuck.md` 那个功能。
...
> **一个功能只有一对文档。** 需求变化、推翻重来、后续轮次，都**在既有文档里追加章节**
> （例：「七、轮次三：……」「### 8.1 R1：……」），**不新开文件** —— 只有"换了一个功能"
> 才开新的一对。文件名里的日期用**首次立项那天**，后续轮次不改名、不搬家。
>
> 反例：同一个功能分两轮各开一对文档，读者得先搞清"该看哪一份"，而且两份都会各自漂移。
> 追加章节时顺带做一件事：**把被本轮推翻的旧表述就地改掉**（设计稿描述的是当前设计，
> 不是历史），历史过程留在实施计划的轮次章节里。
>
> 设计与实施的具体分工：`specs/` 写**当前**设计（会随轮次被改写），
> `plans/` 写**过程**（只追加，不回头改结论）。
...
- [ ] 用户可见的改动 → 新增 `changelog.d/<短名>.md` 片段（**不要直接改 `CHANGELOG.md`**），见下节
...
`CHANGELOG.md` 是**给用户看的变更记录**，不是开发日志。完整约定在本节，`AGENTS.md` 只保留一条指针。

#### 1. 什么时候写、写成什么粒度

| 依据 | 什么时候写 | 粒度 |
| --- | --- | --- |
| GitHub Flow | **不规定**。它只约束「分支 → PR → 评审 → 合入 → 删分支」，变更说明的位置是 PR 描述 | — |
| Keep a Changelog | 途中写进顶部的 `## [Unreleased]`，**发版时**整段搬到新的版本区段 | 一条覆盖多个提交（≈ 一个 PR 一条） |

本仓库的 `CHANGELOG.md` 是**日期表**、**没有 `Unreleased` 区段**，规范里「途中累积」那一段无处安放。等价规则是把**两件事拆开**：

> **内容在 PR 收尾时决定（不可后移），落到 `CHANGELOG.md` 的动作延后到汇总时。**

| 动作 | 性质 | 时机 |
| --- | --- | --- |
| 判断「算不算用户可见、模块叫什么、怎么措辞」 | 内容决策，只有写代码的人有上下文 | **PR 收尾**，产出 `changelog.d/<短名>.md` |
| 往 `CHANGELOG.md` 表格顶部插一行 | 对**共享文件**的写入 | **汇总时**（发版 / 打 tag 前），一次性完成 |

**为什么必须拆**：`CHANGELOG.md` 的约定是「插在表格**最上面**」，只要同时开着 2 个以上 PR，
每个分支都会往**同一个位置**插一行 —— git 三方合并遇到「同一位置的双向插入」**必然冲突**，
与内容是否相同无关。实测一次 4 分支并行：`CHANGELOG.md` 冲突 3 次，而 11 个代码文件 0 冲突。

| 步骤 | 谁做 | 产出 |
| --- | --- | --- |
| **PR 收尾** | PR 作者 | `changelog.d/<短名>.md`（**新文件，永不冲突**；与代码同一个 PR，reviewer 照样看得到） |
| **汇总** | 维护者 | 脚本把片段并入 `CHANGELOG.md` 表格顶部，删除片段，独立提交 |

- **不要跟着每个提交写**：同一个 PR 里连加 5 行是本仓库实际出现过的反例
- **一个 PR 只占一个片段文件**：改了几个模块也合成一行，模块列用 ` / ` 连接，不拆成多行
- **不要直接编辑 `CHANGELOG.md`**：那是汇总步骤的职责，直接改会在并发 PR 时冲突
- **日期与倒序在汇总时确定**：片段里的 `date` 填当天即可，汇总时可用 `--date` 统一覆盖；
  脚本按日期倒序排好再一次性插入，避免并行 PR 各自插「最上面」把倒序打乱

片段格式（`---` 之间是 front matter）：
...
汇总命令（细节见 [`changelog.d/README.md`](../changelog.d/README.md)）：

```bash
python scripts/changelog_fragments.py            # 预览将插入的行（不改文件）
python scripts/changelog_fragments.py --write    # 写入 CHANGELOG.md 并删除已汇总片段
```
...
1. 用户能在界面 / 通知 / 提示文案上看到它吗？
2. 它会改变用户已有的操作习惯吗（开关含义、默认值、入口位置、执行频率）？
3. 出问题时，用户需要靠这条记录对照版本吗？

| 要写 | 不写 |
| --- | --- |
| 新增 / 移除 / 改名的功能与入口 | 开发约定、文档整理、README |
| 界面文案、应用显示名、通知措辞 | CI / 构建脚本、依赖升级（除非影响产物或体积） |
| 设置项的默认值、取值范围、含义变化 | 内部重构、命名调整、注释 |
| 任务调度、请求频率、风控行为变化 | 单测与测试辅助代码 |
| 修复用户能感知的 Bug（卡死、误判、漏执行） | 日志格式 |

判不准时按「**用户会不会因此需要改操作**」定；**宁可写** —— 一行的成本远低于漏记。

#### 3. 一行怎么写

| 列 | 规则 |
| --- | --- |
| 日期 | 该 PR **合入 `main` 的日期**。片段里先填当天；汇总时可用 `--date` 统一覆盖为真实日期 |
| 模块 | 用户能认出的功能名（`设置中心`、`小鸡庄园`），多个用 ` / ` 连接；**不写类名**（`AntForest`） |
| 功能改动 | **用户视角**的变化，用「新增 / 修复 / 调整 / 移除」起头；一句话一个变化，用 `；` 分隔 |

- **正文必须写成一行**：表格单元格不能换行；写进片段文件时也要一行到底
- **正文不能含 `|`**：会破坏表格结构
- **插入位置**：由汇总脚本插到 `CHANGELOG.md` 表格**最上面**（整体按日期倒序，同日相邻）
- **长度**：1~3 句、**≤ 200 字**（现有中位数约 110 字）
- **不写实现**：字段清单、踩坑过程、验证手法放 `docs/superpowers/{specs,plans}/`
- **不追溯**：已超长的历史行保持原样，不为新规则回头改写

自检（校验片段格式与长度，CI 也跑同一条命令）：
