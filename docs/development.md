...
- **不要跟着每个提交写**：同一个 PR 里连加 5 行是本仓库实际出现过的反例（对照：历史惯例一直是一个 PR 一行）
- 1~3 句、200 字以内，只写用户能感知到的变化
- 实现细节、字段清单、踩坑过程写 `docs/superpowers/{specs,plans}/`，不进 CHANGELOG
- 纯开发侧改动（开发约定、CI 配置、内部重构、单测）不占 CHANGELOG 行
