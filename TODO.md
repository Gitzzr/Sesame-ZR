...
- [x] K50 至尊版（小号）已做可回滚的端到端验证：落盘核对通过，随后已还原现场（2026-09-23）
- [x] 切换后广播生效：配置无需重启支付宝即被重载（K50 日志确认）
- [ ] 小米 17（大号）切到大号推荐配置 —— **待用户自行验证，本轮未动该设备**
- [ ] 观察一轮实际任务，确认小号侧不再出现针对好友的能量收取
...
- [x] K50 验证**逐项功能勾选**：一级显示「已启用 12 项」、二级 26 项功能分 4 组且默认按推荐勾选、
      未勾选功能的名单与门控开关均未被误改；核对后已还原配置（2026-09-23）
...
| `2026-09-21-verification-pause-stuck` | 安全验证暂停卡死修复：1009 不再误判为需人工验证、暂停标志加 30 分钟 TTL、通知与支付宝首页确认框新增「跳过并恢复」、正式包接收器注册修复 | 代码全绿（PR #3 已合入），实机待验见 P1-1 |
